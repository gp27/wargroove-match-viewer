...
export function tilesFromLinear({
  tiles,
  width,
}: LinearTilesInput): TilesInput {
...
const terrainConnections: {
  ...
  beach: [
    ...
    ['beach'],
    ['sea', 'ocean', 'reef', 'bridge'],
  ],
  bridge: [
    ['bridge'],
    ['plains', 'forest', 'mountain', 'road', 'wall', 'cobblestone', 'carpet'],
  ],
...
export class PhaserWargrooveMap {
  ...
  private parseTileset(
    ...
    index: PhaserWargrooveMap['tilesetData'] = {}
  ) {
    ...
    for (let id in tileset.tileProperties) {
      ...
      const {
        ...
        terrain,
        group,
      } = tileset.tileProperties[id] as {
        biome: Biome
        terrain: Terrain
        group: string
      }
      const corners: WangIdArray =
        tileset.tileData[id]?.wangid?.connections || Array(8).fill(undefined)
      if (!terrain) continue

      index[terrain] = index[terrain] || {}
      const terrainData = (index[terrain][biome] = index[terrain][biome] || {
        connections: {},
        groups: {},
      })

      let keys = { p: 0, s: 0, t: 0 }
      const connId = corners
        .map((c, i) => {
          return c in keys ? c : '.'
        })
        .join('')

      ;(terrainData.connections[connId] =
        terrainData.connections[connId] || []).push(tileId)

      if (group) {
        const g = (terrainData.groups[group] = terrainData.groups[group] || [])
        g.push([tileId % cols, Math.floor(tileId / cols), tileId])
      }
    }
  ...
  private makeRandomElementsLayer(
    terrain: Terrain,
    biome: Biome,
    tiles: TilesInput
  ) {
  ...
  private generateTerrainConnections(
    terrain: Terrain,
    tiles: TilesInput,
    ...
    return tiles.map((row, y, rows) => {
      return row.map((v, x, cRow) => {
        ...
        let isWaterUnderBridge =
          v == 'bridge' && ['sea', 'river'].includes(terrain)
        ...
        if (isWaterUnderBridge) {
          ...
          connections.forEach((terr, i) => {
            ...
            if (['sea', 'ocean', 'beach', 'reef'].includes(terr))
              seaPoints += point
          })
  ...
  private generateTerrainGroups(
    terrain: Terrain,
    tiles: TilesInput,
    terrainData: TerrainData
  ) {
    const groups = Object.values(terrainData.groups)
  ...
  private generateRandomElements(
    terrain: Terrain,
    tiles: TilesInput,
    elements: typeof terrainElements[Terrain][Biome]
  ) {
    const deltas = [
      [0, 0],
      [0.5, 0.5],
  ...
  private getFrameFromTileId(tileId: number) {
    let {
      tileWidth,
      tileHeight,
      texCoordinates: { [tileId]: { x = 0, y = 0 } = {} as any },
    } = this.tileset
